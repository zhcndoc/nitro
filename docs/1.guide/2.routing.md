...
一个 [事件处理程序](https://h3.unjs.io/guide/event-handler) 是一个将被绑定到路由的函数，当路由被路由器匹配时，将在接收到请求时执行。

:read-more{to="https://h3.unjs.io/guide/event-handler"}

## 文件系统路由

Nitro 支持基于文件的 API 路由（文件会自动映射到 [h3 路由](https://h3.unjs.io/guide/router)）。定义路由就像在 `server/api/` 或 `server/routes/` 目录内创建一个文件一样简单。
...
要定义带参数的路由，使用 `[<param>]` 语法，其中 `<param>` 是参数的名称。参数将可在 `event.context.params` 对象中找到，或使用来自 [unjs/h3](https://h3.unjs.io) 的 `getRouterParam` 工具。
...
在 `server/middleware/` 目录中的中间件会自动注册到所有路由。如果您想为特定路由注册中间件，请参见 [对象语法事件处理程序](https://h3.unjs.io/guide/event-handler#object-syntax)。
...
您可以使用 [H3 中提供的工具](https://h3.unjs.io/guide/event-handler#error-handling) 来处理路由和中间件中的错误。
