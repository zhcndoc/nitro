...
> Nitro v3 现已作为公开测试版发布：这是一次从底层重新演进的服务器框架，围绕 Web 标准、Rolldown、Vite v8 以及随处部署的承诺构建。
...
随着无服务器和边缘计算的兴起，服务器端如今面临着前端多年前解决过的相同限制。**冷启动**意味着启动的每一毫秒都很重要。**内存限制**十分严格，臃肿的依赖可能会让你超出限制。**包体积**会直接影响部署速度和启动时间。而且你的代码需要在任何地方运行：Node.js、Deno、Bun、Cloudflare Workers、Vercel 等。然而，大多数服务器框架仍然提供未经优化、未打包的代码，假定应用会在一个长期运行的进程中运行，而这些问题在这种场景下都不重要。
...
- **API 路由**：通过文件系统路由实现
- **服务端渲染**：与前端构建集成
- **生产服务器**：单次 `vite build` 即可生成经过优化的 `.output/` 文件夹，其中包含前端和后端，可直接部署

这意味着你可以为任何 Vite 项目添加完整的后端。查看使用 [React](/examples/vite-ssr-react)、[Vue](/examples/vite-ssr-vue-router) 和 [Solid.js](/examples/vite-ssr-solid) 的[示例](/examples)。

## 🚀 默认高性能，零膨胀

Nitro 在构建时编译你的路由。运行时没有路由器；每个路由按需加载。只有处理特定请求所需的代码才会被加载和执行。
...
Nitro v3 升级至 [H3 v2](https://h3.dev)，后者围绕 Web 标准原语进行了全面重写：[`Request`](https://developer.mozilla.org/en-US/docs/Web/API/Request)、[`Response`](https://developer.mozilla.org/en-US/docs/Web/API/Response)、[`Headers`](https://developer.mozilla.org/en-US/docs/Web/API/Headers) 和 [`URL`](https://developer.mozilla.org/en-US/docs/Web/API/URL)。
...
任何支持 Web 标准的内容都能与 Nitro 配合使用：[Elysia](https://elysiajs.com/)、[h3](https://h3.dev)、[Hono](https://hono.dev)。
...
::note
当不使用时，不会向服务器包添加任何额外内容。你仍然可以与 Nitro 的内置原语一起使用原生平台原语。
...
与运行时无关的键值层，提供 20 多种驱动：FS、Redis、S3、Cloudflare KV、Vercel Blob 以及[更多](https://unstorage.unjs.io/drivers)。将驱动附加到命名空间，并且无需更改应用代码即可进行切换。
...
无需配置。Nitro 会自动检测你的部署目标。无需更改任何一行代码，即可利用 ISR、SWR 和边缘渲染等平台特性。

## 🎨 服务端渲染

使用你喜欢的模板引擎渲染 HTML，或直接在服务器上使用 React、Vue 或 Svelte 等组件库。通过客户端注水实现完全同构渲染。

Nitro 提供基础和渐进式方案：从 API 路由开始，在需要时添加渲染，并按照自己的节奏扩展到完整的 SSR。
