...
日志详细级别。有关更多信息，请参见 [unjs/consola](https://github.com/unjs/consola?tab=readme-ov-file#log-level)。
...
::note{type=info}
请注意，`defineEventHandler` 是 [`h3`](https://github.com/h3js/h3) 库中的一个助手函数。
::
...
请参见 [unjs/httpxy](https://github.com/unjs/httpxy) 以获取所有可用目标选项。
...
路由选项。它是从路由模式（遵循 [unjs/radix3](https://github.com/h3js/rou3/tree/radix3#route-matcher)）到路由选项的映射。
...
指定构建是否用于 Node.js。如果设置为 `false`，Nitro 会尝试使用 [unjs/unenv](https://github.com/unjs/unenv) 模拟 Node.js 依赖，并调整其行为。
