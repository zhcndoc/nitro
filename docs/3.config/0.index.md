...
- 默认: `false`{lang=ts} (`true`{lang=ts} 当设置了 `DEBUG` 环境变量时)

启用调试模式以获取详细日志记录和额外的开发信息。
...
export default defineNitroConfig({
  serverAssets: [
    {
      baseName: "templates",
      dir: "./templates", // 将 templates/ 作为服务器资产打包
    },
  ],
});
```
...
一个 Nitro 模块数组。模块可以是字符串（路径）、带有 `setup` 函数的对象，或者是函数。
...
WASM 支持配置。请参见 [unwasm](https://github.com/unjs/unwasm) 获取选项。
...
开发服务器选项。您可以使用 `watch` 使开发服务器在指定路径中的任何文件更改时重新加载。
...
开发模式的监视选项。更多信息请参见 [chokidar](https://github.com/paulmillr/chokidar)。
...
- 默认: `{ compressedSizes: true, buildSuccess: true }`{lang=ts}

控制构建日志行为。将 `compressedSizes` 设置为 `false` 以跳过报告压缩后的包大小。将 `buildSuccess` 设置为 `false` 以抑制构建成功消息。
...
默认: `/`{lang=ts}（如果提供了 `NITRO_APP_BASE_URL` 环境变量则使用该值）
...
```ts
export default defineNitroConfig({
  apiBaseURL: "/server/api", // API 路由位于 /server/api/ 下
});
```
...
如果服务器目录中存在 `routes/`、`api/` 或 `middleware/` 目录，它们将自动添加到处理程序数组中。
...
常规处理程序指由打包器导入和转换的处理程序路径。

在某些情况下，我们希望直接提供一个带有编程使用的处理程序实例。

我们可以使用 `devHandlers`，但请注意它们**仅在开发模式下可用**，**不在生产构建中**。
...
export default defineNitroConfig({
  firebase: {
    gen: 2, // 使用 Cloud Functions 2nd gen
    region: "us-central1",
  },
});
```
