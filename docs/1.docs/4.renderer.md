...
export default defineConfig({
  renderer: {
    ...
    static: false,             // Force dynamic template processing (auto-detected when unset)
  }
})
```
...
默认情况下，Nitro 会自动在项目根目录或扫描目录之一中查找 `index.html` 文件。
...
对于动态模板，Nitro 使用 [rendu](https://github.com/h3js/rendu) 超文本预处理器，这是一种使用 JavaScript 表达式创建动态 HTML 模板的简单而强大的方式。

#### 输出表达式

- `{{ expression }}`：HTML 转义输出
...
::note
如果您定义了自己的捕获所有路由（`[...].ts`），它会与其他 `/**` 处理程序串联，而不是取代它们：捕获所有处理程序会按照注册顺序运行（基于文件的捕获所有处理程序，然后是[服务器入口](/docs/server-entry)，最后是渲染器），第一个返回响应的处理程序获胜。渲染器只会处理您的捕获所有路由未作出响应的请求。
::
