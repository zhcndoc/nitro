...
## Auto Imports Removed

Nitro v2 supported auto-imports (powered by [unimport](https://github.com/unjs/unimport)), configurable via the `imports` option, exposing utilities from `server/utils/` and a virtual `#imports` module.

This feature had been removed. The `imports` config option, the `#imports` module and the generated `nitro-imports.d.ts` types are no longer available.
...
If `experimental.database` is enabled, `useDatabase` is no longer auto-imported either; import it from `nitro/database`.
