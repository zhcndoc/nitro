...
任何文件都可以通过 `type` 导入属性进行导入。其内容会以内联方式添加到服务器构建包中：使用 [`type: "bytes"`](https://github.com/tc39/proposal-import-bytes) 时为 `Uint8Array`，或使用 [`type: "text"`](https://github.com/tc39/proposal-import-text) 时为 UTF-8 `string`：
...
或者，`raw:` 前缀会将文件导入为 `string`（对于文本文件，根据文件类型确定）或 `Uint8Array`（对于二进制文件）：
...
> [!NOTE]
> 导入的文件会以内联方式添加到服务器构建包中（二进制文件会进行 base64 编码）。对于较大的文件，建议使用[服务器资源](#server-assets)。
