...
- `maxAge` -- Cache `max-age` in seconds. When set, a `Cache-Control: public, max-age=<value>, immutable` header is applied via route rules. Only takes effect when `fallthrough` is `false` (the default for non-root directories).
...
::note
Only compressible MIME types (text, JavaScript, JSON, XML, WASM, fonts, SVG, etc.) with a file size of at least 1 KB are compressed. Source map files (`.map`) are excluded. zstd compression requires Node.js 22+ (`zlib.zstdCompress`) at build time and is silently skipped on older versions.
::
...
::tip
If your code never uses `useStorage()`, server assets won't be included in the server bundle.
::
...
Server assets are accessed through the storage layer. See the storage docs for the full list of available methods.
::
