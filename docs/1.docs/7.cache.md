...
To use a persistent backend, set the `cache` mount point using the `kv` option:
...
In development, you can also overwrite the cache mount point using the `$development` config key:
...
To also override it while prerendering, set it inside `$prerender` as well.
