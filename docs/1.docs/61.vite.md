...
Nitro does not depend on `vite` itself: it resolves the one installed in your project. A framework running Vite programmatically should pass its own `vite` package with the `vite.path` option, so the dev module runner is created from the Vite instance that is actually running (in a monorepo, a different version can be hoisted next to the app):
