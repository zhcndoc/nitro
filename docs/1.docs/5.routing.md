...
它是一个从路由模式（遵循 [rou3](https://github.com/h3js/rou3)）映射到路由选项的映射，并基于 [`h3-rules`](https://github.com/h3js/h3-rules)。

当设置 `cache` 选项时，匹配该模式的处理程序会自动使用 `defineCachedHandler` 进行包装。请参阅[缓存指南](/docs/cache)以进一步了解此函数。
...
传入对象可以进行更精细的控制，例如来源允许列表、`credentials`、`maxAge` 等（h3 的 `CorsOptions`）。将 `credentials: true` 与通配符来源结合使用是无效的，并会在构建时抛出错误：
