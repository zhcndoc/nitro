...
Nitro 请求处理器是一个接受 `event` 对象的函数，`event` 是一个 [H3Event](https://h3.dev/guide/api/h3event#h3event-properties) 对象。
...
除了文件系统路由，您还可以使用 `routes` 配置选项以程序化方式注册路由处理器。

### `routes` 配置

`routes` 选项允许您将路由模式映射到处理器：
...
每个路由条目可以是一个简单的字符串（处理器路径），或带有以下选项的对象：
