...
WebSocket enables real-time, bidirectional communication between client and server. Nitro's WebSocket integration works across all supported deployment targets including Node.js, Bun, Deno, Vercel and Cloudflare Workers.

::tip
If you only need one-way server-to-client updates (notifications, live feeds), consider [Server-Sent Events](#server-sent-events-sse) instead. They use plain HTTP and reconnect automatically.
::
...
| `protocol` | `string` | Select the WebSocket subprotocol for the connection (must be one of the values the client offered in `Sec-WebSocket-Protocol`) |
| `namespace` | `string` | Override the pub/sub namespace for this connection |
| `context` | `object` | Data attached to `peer.context` |
| `handled` | `boolean` | When `true`, signals that the hook already performed the upgrade itself, so the adapter skips its own upgrade handling |
...
::note
Depending on the adapter, additional hooks such as `drain`, `ping`, and `pong` may be available. See the [CrossWS compatibility table](https://crossws.h3.dev/guide/peer#compatibility).
::
