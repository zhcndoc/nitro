...
WebSocket 支持客户端与服务器之间进行实时双向通信。Nitro 的 WebSocket 集成适用于所有受支持的部署目标，包括 Node.js、Bun、Deno、Vercel 和 Cloudflare Workers。

::tip
如果你只需要单向的服务器到客户端更新（通知、实时信息流），可以考虑改用 [Server-Sent Events](#server-sent-events-sse)。它们使用普通 HTTP，并会自动重新连接。
::

:read-more{to="https://crossws.h3.dev/" title="CrossWS 文档"}
...
| 属性 | 类型 | 说明 |
|---|---|---|
| `headers` | `HeadersInit` | 要包含在升级响应中的 Response headers |
| `protocol` | `string` | 为连接选择 WebSocket 子协议（必须是客户端在 `Sec-WebSocket-Protocol` 中提供的值之一） |
| `namespace` | `string` | 覆盖此连接的发布/订阅命名空间 |
| `context` | `object` | 附加到 `peer.context` 的数据 |
| `handled` | `boolean` | 为 `true` 时，表示钩子已经自行完成升级，因此适配器会跳过自身的升级处理 |
...
::note
根据适配器的不同，可能还会提供其他钩子，例如 `drain`、`ping` 和 `pong`。请参阅 [CrossWS 兼容性表](https://crossws.h3.dev/guide/peer#compatibility)。
::
...
默认情况下，命名空间根据请求 URL 的路径名派生。它与[动态路由](/docs/routing#dynamic-routes)配合使用时非常自然：每个路径都有自己的隔离命名空间：
...
在此示例中，连接到 `/rooms/game` 的客户端与连接到 `/rooms/lobby` 的客户端相互隔离；每个路径都是自己的命名空间。
...
:read-more{to="https://h3.dev/" title="H3 文档"}
