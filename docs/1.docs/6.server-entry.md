...
当服务端入口的默认导出为普通对象时，除 `fetch` 之外的每个属性都会原样传递给由 [Node.js](/deploy/runtimes/node)、[Bun](/deploy/runtimes/bun) 和 [Deno](/deploy/runtimes/deno) 预设启动的 [srvx](https://srvx.h3.dev/) 服务器。这让你可以控制服务器本身：针对每个请求运行的 `middleware` 和 `plugins`（在 Nitro 之前）、`tls`、`maxRequestBodySize`、`trustProxy`、`gracefulShutdown`、运行时特定设置（`node`、`bun`、`deno`）等。
...
::note
- `NITRO_PORT`/`PORT`、`NITRO_HOST`/`HOST` 和 `NITRO_SSL_CERT`/`NITRO_SSL_KEY` 环境变量的优先级高于 `port`、`hostname` 和 `tls` 选项，因此服务器可以在运行时进行配置
- 在开发过程中（`nitro dev`），选项会应用于开发 worker 服务器，但监听器选项（`port`、`hostname`、`protocol`、`tls`、`silent`、`gracefulShutdown`）由开发服务器和 CLI（`--port`、`--host`）控制。Vite 开发服务器不会应用其中任何选项
- 不支持 `manual`：预设会立即开始监听
- 选项只会从普通对象导出中读取（不会从 `export default app` 等框架实例中读取），并且永远不会从 Node.js 格式的入口（`server.node.ts`）中读取
::
