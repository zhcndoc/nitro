...
1. [创建新的 Web 服务](https://dashboard.render.com/select-repo?type=web) 并选择包含您代码的仓库。
2. 确保已选择 'Node' 环境。
3. 将启动命令更新为 `node .output/server/index.mjs`
4. 点击 'Advanced' 并添加一个环境变量，将 `NITRO_PRESET` 设置为 `render_com`。您可能还需要添加一个 `NODE_VERSION` 环境变量，设置为 `20` 以确保构建成功 ([文档](https://render.com/docs/node-version))。
