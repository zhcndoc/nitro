...
```tsx [app/root.tsx]
import "./index.css"; // css 导入会自动注入导出的服务器组件
    ...
    <div id="root">
      <div>
        ...
        <a href="https://v3.nitro.build" target="_blank">
          <img src={nitroLogo} className="logo" alt="Nitro 标志" />
        </a>
      </div>
      ...
      <ul className="read-the-docs">
        <li>
          编辑 <code>src/client.tsx</code> 以测试客户端热更新。
        </li>
        <li>
          编辑 <code>src/root.tsx</code> 以测试服务器端热更新。
        </li>
        <li>
          访问{" "}
...
async function main() {
  // 保存 `setPayload` 函数以从 `BrowserRoot` 组件外部触发重新渲染（例如服务器函数调用、导航、热更新）
  let setPayload: (v: RscPayload) => void;

  // 反序列化 RSC 流回 React 虚拟 DOM 以支持 CSR
  const initialPayload = await createFromReadableStream<RscPayload>(
    // 初始 RSC 流被注入在 SSR 流的 <script>...FLIGHT_DATA...</script> 中
    rscStream
  );

  // 浏览器根组件，将 RSC 负载作为状态进行（重新）渲染
  ...
  // 注册一个处理器，会在 React 水合后被服务器函数请求内部调用
  ...
  // 实现服务器热更新，通过监听服务器代码变动触发重新fetch和渲染 RSC
  if (import.meta.hot) {
...
// RSC 环境中序列化到 RSC 流的数据结构
// 以及 SSR/客户端环境中反序列化
export type RscPayload = {
  // 此演示渲染/序列化/反序列化整个根 HTML 元素
  // 但此机制可以根据自己的路由约定，调整为渲染/获取不同组件部分。
  root: React.ReactNode;

  // 非渐进增强情况下的服务器操作返回值
  returnValue?: { ok: boolean; data: unknown };

  // 渐进增强情况下服务器操作的表单状态（例如 useActionState）
  formState?: ReactFormState;
};

// 默认假定 `rsc` 入口导出默认的请求处理函数，
// 但服务器入口的执行方式可以通过注册自定义服务器处理函数进行定制。
export default async function handler(request: Request): Promise<Response> {
  ...
  if (renderRequest.isAction === true) {
    if (renderRequest.actionId) {
      ...
      // 否则服务器函数通过 `<form action={...}>` 调用
      // 发生于水合前（如禁用 JavaScript）
      // 即渐进增强方式。
  ...
  // 从 React 虚拟 DOM 树序列化到 RSC 流
  // 在处理服务器函数请求后渲染 RSC 流，
  // 以保证新的渲染反映服务器函数调用后的更新状态，
  // 实现单次往返完成服务器状态变更及获取。
  ...
  // 根据 `RenderRequest` 决定响应 RSC 流，不进行 HTML 渲染
  ...
  // 委派给 SSR 环境完成 HTML 渲染。
  // 插件提供 `loadModule` 辅助加载 SSR 环境入口模块，
  // 但可根据需求自定义运行时通信，比如 `@cloudflare/vite-plugin` 服务绑定。
...
// 最简 ErrorBoundary 示例，浏览器端全局错误处理
  ...
  return (
    <html>
      ...
      >
        ...
        <pre>
          错误:{" "}
          {import.meta.env.DEV && "message" in props.error ? props.error.message : "(未知错误)"}
        </pre>
        <button
          onClick={() => {
...
// - 使用 `x-rsc-action` 头传递服务器操作 ID
const URL_POSTFIX = "_.rsc";
const HEADER_ACTION_ID = "x-rsc-action";

// 用于在 RSC/SSR 渲染和操作处理之间路由的解析请求信息。
// 由 parseRenderRequest() 从入站 HTTP 请求创建。
type RenderRequest = {
  isRsc: boolean; // 如果请求应返回 RSC 负载（通过 _.rsc 后缀），为 true
  isAction: boolean; // 如果是服务器操作调用（POST 请求），为 true
  actionId?: string; // 来自 x-rsc-action 头的服务器操作 ID
  request: Request; // URL 去除 _.rsc 后缀的标准化请求
  url: URL; // 去除 _.rsc 后缀的标准化 URL
};
...
export function parseRenderRequest(request: Request): RenderRequest {
  ...
  if (url.pathname.endsWith(URL_POSTFIX)) {
    ...
    if (request.method === "POST" && !actionId) {
      throw new Error("缺少 RSC 操作请求的 action id 头");
    }
    return {
      isRsc: true,
      isAction,
