...
const fixtureDir = fileURLToPath(new URL("./alias-precedence-fixture", import.meta.url));
const tmpDir = fileURLToPath(new URL("./alias-precedence-fixture/.tmp", import.meta.url));
...
describe("alias-precedence", () => {
  for (const builder of ["rolldown", "rollup", "vite"] as const) {
    describe(builder, () => {
      ...
      it("uses the most specific alias key", async () => {
